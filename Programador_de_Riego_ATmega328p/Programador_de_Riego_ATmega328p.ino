...
//este es el bueno adaptado para PANTALLA DE 2.4
#include <SPI.h>
#include <Wire.h>
...
//Inicializar por primera vez la eeprom del ATmega328p
 ...
 tft.begin(0x9341);
   tft.setRotation(1);
   tft.setAddrWindow(0, 0, 240, 320); // Ajustar la resolución a 240x320
...
void limpiarSelectorSecuencia(){
  tft.setTextColor(WHITE,WHITE);
  textSinClear(18, 55, ">", 2, NULL);
  textSinClear(18, 95, ">", 2, NULL);  
  textSinClear(18, 135, ">", 2, NULL);
  textSinClear(18, 175, ">", 2, NULL);  
  textSinClear(18, 215, ">", 2, NULL);   
  tft.setTextColor(BLACK,WHITE);
  }
...
void settings(){  
    ...
    textSinClear(50, 12, "CONFIGURAR SISTEMA", 2, NULL);  
    tft.setTextColor(BLACK,WHITE);        
...
void dashboard(){
   ...
   if (digitalRead(valvula_1)==LOW) {
      tft.setTextColor(BLACK,WHITE); 
      textSinClear(110, 190, "REGANDO!", 2, NULL);
   }
   else {
    tft.setTextColor(WHITE,WHITE); 
    textSinClear(110, 190, "REGANDO!", 2, NULL);
...
void subDashboard2(char diaSemanaArray[], byte pos_Y, byte HoraInicio, byte MinutoInicio, byte DuracionHora, byte DuracionMinuto, boolean fModo){
  tft.setTextColor(WHITE);   
  textSinClear(31, pos_Y, "D:",1 , NULL);
...
void fechaHora2(){
   ...
   textSinClear(90, 12, "FECHA Y HORA", 2, NULL);   
  ...
  //tft.setFont(&FreeSevenSegNumFont);
   ...
   textSinClear(170, 80, "-", 3, NULL); 
   
   textSinClear(195, 80, "20", 3, NULL);          

  // tft.setFont(&FreeSevenSegNumFont);
   tft.setCursor(232, 80);
   tft.setTextSize(3);         
   tft.print(anio);
  ...
  // tft.setFont(&FreeSevenSegNumFont);
   ...
   textSinClear(200, 185, "GUARDAR", 2, NULL); 
...
void fechaHora(){
  ...
  JoystickY = analogRead(A7);
  i=1;   
  dia = edit(60, 80, 3, dia, NULL);                      // Edit date
  mes = edit(130, 80, 3, mes, NULL);                    // Edit month
  anio = edit(232, 80, 3, anio, NULL);                    // Edit year
  hora = edit(120, 130, 3, hora, NULL);                     // Edit hours
  minuto = edit(175, 130, 3, minuto, NULL);                   // Edit minutes
  segundo = 0;
    while(true){
      JoystickX = analogRead(A6);
      //JoystickY = analogRead(A7);
...
void riegoManual(){
  ...
  tft.fillRect(0,0,480,48,BLACK);
  tft.setTextColor(WHITE);     
  textSinClear(85, 12, "RIEGO MANUAL", 2, NULL);
  tft.setTextColor(BLACK,WHITE);         
  ...
  while(!flagDashboard){
    ...
    if (!digitalRead(pulsador)){
      ...
      switch(IDSubSetting){
        case 1:
        tft.fillScreen(WHITE); 
        textSinClear(75, 100, "ACTIVADO", 4, NULL); 
        delay(1500);
        ...
        default:  
        tft.fillScreen(WHITE); 
        textSinClear(25, 100, "DESACTIVADO", 4, NULL); 
        delay(1500); 
...
void mostrarSectoresActivar(){
  ...
  tft.fillRect(0,0,480,48,BLACK);
  tft.setTextColor(WHITE);     
  textSinClear(75, 12, "ACTIVAR SECTORES", 2, NULL);
  tft.setTextColor(BLACK,WHITE); 
  textSinClear(45, 80, "(  ) SECTOR #1", 2, NULL); 
  tft.setCursor(64, 80);
  tft.print(sector_Marca[0]);

  textSinClear(220, 210, "GUARDAR", 2, NULL);    
}

void mostrarSectores(){
  switch(contadorSector){
  case 1: textSinClear(105, 80, "SECTOR #1", 2, NULL); break;
  default: textSinClear(220, 210, "GUARDAR", 2, NULL);              
  }
}  
...
void selectorProgSecuencia(){
  switch(countJoystickV){
  case 1: limpiarSelector(); textSinClear(18, 80, ">", 2, NULL); IDSubSetting=1; break;
  case 2: limpiarSelector(); textSinClear(18, 120, ">", 2, NULL); IDSubSetting=2;   
  }
}
...
void menuProgramarSecuencia(){
  ...
  tft.fillRect(0,0,480,38,BLACK);
  tft.setTextColor(WHITE);     
  textSinClear(50, 15, "PROGRAMAR SECUENCIA", 2, NULL);    
  tft.setTextColor(BLACK,WHITE);        
  textSinClear(45, 80, "SECTOR #1", 2, NULL);
  textSinClear(45, 120, "SALIR", 2, NULL);    
...
void menuProgramarSecuencia2(){
  ...
  textSinClear(50, 15, "PROGRAMAR SECUENCIA", 2, NULL);    
  tft.setTextColor(BLACK,WHITE);        
  textSinClear(45, 55, "HORA DE INICIO", 2, NULL);
  textSinClear(45, 95, "DURACION", 2, NULL);
  textSinClear(45, 135, "DIAS DE RIEGO", 2, NULL);
  textSinClear(45, 175, "BORRAR SECUENCIA", 2, NULL);  
  textSinClear(45, 215, "ATRAS", 2, NULL);
  countJoystickV=1;
...
void selectorProgSecuencia2(){
  switch(countJoystickV){
  case 1: limpiarSelectorSecuencia(); textSinClear(18, 55, ">", 2, NULL); IDSubSetting=1; break;
  case 2: limpiarSelectorSecuencia(); textSinClear(18, 95, ">", 2, NULL); IDSubSetting=2; break;
  case 3: limpiarSelectorSecuencia(); textSinClear(18, 135, ">", 2, NULL); IDSubSetting=3; break;
  case 4: limpiarSelectorSecuencia(); textSinClear(18, 175, ">", 2, NULL); IDSubSetting=4; break;
  default: limpiarSelectorSecuencia(); textSinClear(18, 215, ">", 2, NULL); IDSubSetting=5;   
  }
}
...
void agregarhoraInicio(byte horaInicio, byte minutoInicio, byte numPrograma){
    ...
    tft.fillRect(0,0,480,38,BLACK);
    tft.setTextColor(WHITE);     
    textSinClear(75, 12, "HORA DE INICIO", 2, NULL);    
    tft.setTextColor(BLACK,WHITE);   
  while(true){
    horarioSecuencia(horaInicio, minutoInicio);
    i=4; 
    horaInicio = edit(62, 140, 1, horaInicio,&FreeSevenSegNumFont);
    if(horaInicio==24) {
      horaInicio=0;
    ...
    i=5;
    minutoInicio = edit(189, 140, 1, minutoInicio,&FreeSevenSegNumFont);                
    if(minutoInicio==60) {
      minutoInicio=0;
    ...
    while(true){
      ...
      //JoystickY = analogRead(A7);
      tft.setTextColor(WHITE, WHITE); 
      textSinClear(175, 180, "GUARDAR", 3, NULL);   
      oscilarText();
      tft.setTextColor(BLACK, WHITE); 
      textSinClear(175, 180, "GUARDAR", 3, NULL); 
      oscilarText();
...
void horarioSecuencia(byte horaInicio, byte minutoInicio){   
   textSinClear(60, 60, "HORA", 3, NULL);
   textSinClear(170, 60, "MINUTO", 3, NULL);    
   tft.setFont(&FreeSevenSegNumFont);
   tft.setTextSize(1);     
   tft.setCursor(62, 140);// HORA        
   if(horaInicio==24) printDigits(horaInicio-24);    
   else {printDigits(horaInicio);}   
   textSinClear(140, 90, ":", 6, NULL);
   tft.setFont(&FreeSevenSegNumFont);   
   tft.setTextSize(1);   
   tft.setCursor(189, 140); //MINUTO
...
void agregarDuracion(byte duracionHoras, byte duracionMinutos, byte numPrograma){
    ...
    tft.fillRect(0,0,480,38,BLACK);
    tft.setTextColor(WHITE);     
    textSinClear(110, 12, "DURACION", 2, NULL);    
    tft.setTextColor(BLACK,WHITE);  
  while(true){
    duracionSecuencia(duracionHoras, duracionMinutos);
    i=4;
    duracionHoras=edit(62, 140, 1, duracionHoras, &FreeSevenSegNumFont); //HORA
    if(duracionHoras==24) {
      duracionHoras=0;
    ...
    i=5;
    duracionMinutos=edit(189, 140, 1, duracionMinutos, &FreeSevenSegNumFont); //MINUTOS
...
//POSICION DEL CURSOSR EN LA PANTALLA DIAS DE RIEGO MODIFICAR AQUI Y EN LA FUNCION mostrarDiasRiego en el serCursor
void selectorDiasRiego(byte diasRiego[], char marcaDiasRiego[], char diaSemana[]){
  switch(diaSemanaRiego){
    case 1:
    if(diasRiego[0]==HIGH) {
      marcaDiasRiego[0]='X'; diaSemana[0]='L'; // LUNES
    ...
    case 2:
    if(diasRiego[1]==HIGH) {
      marcaDiasRiego[1]='X'; diaSemana[1]='M'; //MARTES
    ...
    case 3:
    if(diasRiego[2]==HIGH) {
      marcaDiasRiego[2]='X'; diaSemana[2]='M'; //MIERCOLES
    ...
    case 4:
    if(diasRiego[3]==HIGH) {
      marcaDiasRiego[3]='X'; diaSemana[3]='J'; //JUEVES
...
//DIAS DE RIEGO INTERFAZ
void mostrarDiasRiego(char marcaDiasRiego[]){
  tft.fillScreen(WHITE);
  tft.fillRect(0,0,480,38,BLACK);
  tft.setTextColor(WHITE);     
  textSinClear(75, 12, "DIAS DE RIEGO", 2, NULL);    
  tft.setTextColor(BLACK,WHITE); 
  textSinClear(10, 50, "(  ) LUNES", 2, NULL); 
  tft.setCursor(29, 50);
  tft.print(marcaDiasRiego[0]);
  textSinClear(10, 90, "(  ) MARTES", 2, NULL);
  tft.setCursor(29, 90);
  tft.print(marcaDiasRiego[1]);       
  textSinClear(10, 130, "(  ) MIERCOLES", 2, NULL);  
  tft.setCursor(29, 130);
  tft.print(marcaDiasRiego[2]);           
  textSinClear(10, 170, "(  ) JUEVES", 2, NULL);  
  tft.setCursor(29, 170);
  tft.print(marcaDiasRiego[3]);    
  textSinClear(10, 210, "(  ) VIERNES", 2, NULL);
  tft.setCursor(29, 210);
  tft.print(marcaDiasRiego[4]);     
  textSinClear(160, 50, "(  ) SABADO", 2, NULL);  
  tft.setCursor(178, 50);
  tft.print(marcaDiasRiego[5]);        
  textSinClear(160, 90, "(  ) DOMINGO", 2, NULL); 
  tft.setCursor(178, 90);
  tft.print(marcaDiasRiego[6]);          
  textSinClear(220, 210, "GUARDAR", 2, NULL);    
}
...
void mostrarDiasRiego2(){
  switch(diaSemanaRiego){
  case 1: textSinClear(70, 50, "LUNES", 2, NULL); break;
  case 2: textSinClear(70, 90, "MARTES", 2, NULL); break;
  case 3: textSinClear(70, 130, "MIERCOLES", 2, NULL); break;
  case 4: textSinClear(70, 170, "JUEVES", 2, NULL); break;
  case 5: textSinClear(70, 210, "VIERNES", 2, NULL); break;
  case 6: textSinClear(220, 50, "SABADO", 2, NULL); break;
  case 7: textSinClear(220, 90, "DOMINGO", 2, NULL); break;
  default: textSinClear(220, 210, "GUARDAR", 2, NULL);              
  }
}  
...
void borrarSecuencia(byte numPrograma){
  ...
  tft.fillScreen(WHITE);
  tft.setTextColor(BLACK, WHITE);   
  textSinClear(50, 75, "SECUENCIA BORRADA", 2, NULL);
  delay(1500);  
}
